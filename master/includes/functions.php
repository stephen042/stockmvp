...
include __DIR__ . "/../../mailer/Exception.php";
	...
	function sendMail($email, $subject, $message){
		...
		$site_name = "StockMVP";
		$mail_host = "server270.web-hosting.com";
		$mail_username = "support@stockmvp.online";
		$mail_password = "StockmvpSecret.";
		$mail_sendFrom = "support@stockmvp.online";
	...
	function after_successful_login($admin_id='') {
	...
	function bdate_toText($datetime=""){
		$nicetime = date("Y-M-d H:i A", strtotime($datetime));
		return $nicetime;	
	...
	function checkVerificationStatus($user_id){
		global $conn;
		$query = $conn->query("SELECT * FROM account WHERE user_id = '$user_id' LIMIT 1");
		$row = $query->fetch();
